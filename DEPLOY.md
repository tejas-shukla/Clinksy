...
A step-by-step guide to taking Clinksy from your laptop to a live URL on its own
...
## Step 1 — Push Clinksy to its own GitHub repo

Clinksy has its own fresh git history, independent of Doorstep. From the Clinksy
folder:

```bash
cd "/Users/tejas/Documents/Claude/Projects/New Home Website/Clinksy"
git add .
git commit -m "Initial Clinksy commit"
```

Create a **new** repository on GitHub called `clinksy` (keep it separate from the
`doorstep` repo), then push:

```bash
git remote add origin https://github.com/<your-username>/clinksy.git
git branch -M main
...
2. Click **Import Git Repository** and pick your **`clinksy`** repo (not doorstep).
3. Vercel auto-detects Next.js. Leave the defaults.
4. Add the environment variables below.
5. Click **Deploy**.

This creates a brand-new project with its own URL, e.g.
`https://clinksy-abc123.vercel.app`, completely separate from the Doorstep
deployment.
...
## Step 4 — Custom domain (Clinksy)

Once you're happy with the `.vercel.app` preview:

1. In the Vercel project go to **Settings → Domains**.
2. Add your Clinksy domain (e.g. `clinksy.com`).
...
The whole point of Clinksy is to be a safe copy: make and check changes here,
confirm they look right on the Clinksy `.vercel.app`, and only then mirror the
